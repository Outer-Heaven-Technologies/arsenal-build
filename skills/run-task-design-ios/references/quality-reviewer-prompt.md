...
Used by `run-task-design-ios` for the code quality review step. Runs only after visual fidelity and spec compliance have both passed. Spawn with this prompt:
...
- Issues that fall outside Swift/SwiftUI craft (e.g., "this copy doesn't match the brand voice") belong to the `clarify` finisher in `run-task-design-ios`'s finisher pass step, not here.
