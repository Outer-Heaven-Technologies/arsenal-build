...
The task's tags (`domain:`, `research:`) are re-read from `docs/TASKS.md` directly so direct invocation works without orchestrator state. If the task's `domain:` is not `design`, report `WRONG_DOMAIN` and stop — that task belongs to `run-task-feature`.
