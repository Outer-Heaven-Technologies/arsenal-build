...
Used by `run-task-feature-web` for the spec compliance review step. Runs after the feature-implementer reports DONE. Reviewer's job is to verify the code matches the task specification AND enforces the component-boundary rule — not to assess code quality (that's the downstream quality review step). Spawn with this prompt:
