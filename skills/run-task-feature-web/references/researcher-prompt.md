...
Used by `run-task-feature-web` for the researcher dispatch step when a task is flagged `research: yes`. Spawn a fresh-context researcher subagent with this prompt, substituting the bracketed placeholders with task-specific values.
