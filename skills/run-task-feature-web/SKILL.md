...
| Design-pipeline commits already on the phase branch | `close-design-phase-web` upstream | The components this task may wire to are reachable in git history; the context brief's available-components manifest enumerates them |
...
The task's tags (`domain:`, `research:`) are re-read from `docs/TASKS.md` directly so direct invocation works without orchestrator state. If the task's `domain:` is not `feature`, report `WRONG_DOMAIN` and stop — that task belongs to `run-task-design-web`.
...
If the brief is missing, report `NEEDS_CONTEXT` and tell the caller to run `/arsenal-build:generate-feature-briefs --phase N --task N --surface web` first.
...
- **Status:** DONE | SKIPPED (M1 idempotence) | WRONG_DOMAIN (task is `domain: design`, route to `run-task-design-web`) | NEEDS_CONTEXT (cite which brief is missing) | BLOCKED (cite the implementer's BLOCKED reason and your assessment)
- **Phase:** N
- **Task:** N
...
- **Deferred minor issues:** list (these aggregate into the phase summary at `close-feature-phase-web`)
...
| Skill | Relationship |
|---|---|
| `/arsenal-build:features-web` | Invokes this skill in a loop, one call per `- [ ]` task in the phase's `### Feature tasks` subsection. Hand-off: phase number + task identifier. Runs after `close-design-phase-web` has returned. |
...
| `/arsenal-build:close-design-phase-web` | Runs **before** the orchestrator invokes this skill. Commits the components this skill's tasks may wire to. |
| `/arsenal-build:close-feature-phase-web` | Runs **after** all feature tasks in the phase complete. Aggregates deferred Minor issues this skill reported per task, runs the test suite + CodeRabbit + push + PR. |
