...
- **Estimated implementer complexity:** Mechanical | Integration | Architectural (drives downstream model selection in `run-task-design-web`)
```
...
- `generate-design-briefs` confirms the file was written but does not read its contents. Downstream `run-task-design-web` reads it by path.
