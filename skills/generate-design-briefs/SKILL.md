...
- `--surface web|ios` — surface flag (required), selects the design-brief subagent template
...
- `docs/TASKS.md` — to discover task tags (`domain:`, `research:`, plus iOS-only `finishers:` and `snapshot:` on design-domain tasks) and locate each `domain: design` task under the phase's `### Design tasks` subsection. Re-read here rather than trusting orchestrator state.
...
2. Parse `research: yes/no`, plus on iOS `finishers: [list]` / `snapshot: yes/no` (both apply only to design-domain tasks).
...
- **Component creation path** — declared by `DESIGN_SYSTEM.md` or by project convention (e.g., `components/ui/<ComponentName>.tsx` for web, `Views/<ScreenName>.swift` for iOS). The design implementer writes the component at this path; downstream feature tasks reference it as a read-only input.
...
Dispatch a fresh-context design-brief subagent using the surface-appropriate template:

- `--surface web` → `references/design-brief-prompt-web.md`
- `--surface ios` → `references/design-brief-prompt-ios.md`
...
The controller (this skill) does **not** read the design brief contents after the subagent writes it — only confirms the file exists. Downstream `run-task-design-{web,ios}` reads the brief via path reference.
...
The caller's next step is to execute each design task via `run-task-design-{web,ios}`; this skill does not run tasks itself.

## Reference files

| File | Used in | Purpose |
|---|---|---|
| `references/design-brief-prompt-web.md` | Step 2b (web) | Template for the design-brief subagent on web — translates mockup + DESIGN_SYSTEM into a token-mapped technical plan with variant matrix. |
| `references/design-brief-prompt-ios.md` | Step 2b (iOS) | Template for the design-brief subagent on iOS — translates mockup region → SwiftUI surface with theme-token map, primitive cites, variant coverage, finisher anchors, navigation script, and gap flags. |
...
- **Don't read brief contents after writing them.** Controller confirms the design brief subagent's file exists and reports DONE. Downstream `run-task-design-{web,ios}` reads briefs by path.
...
| Skill | Relationship |
|---|---|
| `/arsenal-build:design-web` | Invokes this skill at Step 2 with `--surface web` after `expand-phase`. Hand-off: phase number, surface flag. |
| `/arsenal-build:design-ios` | Same, with `--surface ios`. |
| `/arsenal-build:expand-phase` | Runs **before** this skill. Writes the concrete tagged task list this skill reads from `TASKS.md`. |
| `/arsenal-build:generate-feature-briefs` | Sibling skill. Runs **after** this skill **and after** `close-design-phase-{web,ios}` — the feature briefs reference the components this design pipeline commits. This skill never touches feature-domain tasks. |
| `/arsenal-build:run-task-design-web` / `/arsenal-build:run-task-design-ios` | Runs **after** this skill, per task. Reads briefs from `.tasks/phase-{N}/` by path. |
