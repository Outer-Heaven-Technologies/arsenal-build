...
Used by `run-task-feature` for the researcher dispatch step when a task is flagged `research: yes`. Spawn a fresh-context researcher subagent with this prompt, substituting the bracketed placeholders with task-specific values.

```
## Research Task
[The specific question to research — e.g., "Stripe webhook idempotency semantics and replay-safety patterns in Next.js App Router"]
