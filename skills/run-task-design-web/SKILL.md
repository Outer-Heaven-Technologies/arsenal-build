...
Visual fidelity is **static analysis** on web — the reviewer reads the design brief + the implemented code + the mockup and checks adherence without a live browser. (A live `claude-in-chrome` audit lives at `close-design-phase-web` later in the phase, run once across all surfaces shipped.)
...
|---|---|---|
| `docs/TASKS.md` with phase block in concrete-tasks state | `expand-phase` upstream | Read the task line and tags (`domain: design`, `research:`); flip `[ ]` → `[x]` after success |
| `.tasks/phase-{N}/task-{N}-context.md` | `generate-design-briefs` upstream | Design-implementer reads it by path |
| `.tasks/phase-{N}/task-{N}-design.md` | `generate-design-briefs` upstream | Design-implementer + visual-fidelity reviewer read it; the brief carries the token map, variant matrix, and mockup pointer |
| `docs/CONVENTIONS.md`, `docs/ARCHITECTURE.md`, `docs/DESIGN_SYSTEM.md`, `docs/DESIGN.md` | `anchor-files` upstream | Excerpted into the briefs upstream — this skill does not re-read them in full |
| `docs/mockups/<screen>.{jsx,tsx,html,png,figma-export.json}` (when applicable) | Project | Visual fidelity reviewer compares the implemented code to the mockup at the cited region |
...
The task's tags (`domain:`, `research:`) are re-read from `docs/TASKS.md` directly so direct invocation works without orchestrator state. If the task's `domain:` is not `design`, report `WRONG_DOMAIN` and stop — that task belongs to `run-task-feature-web`.

## Workflow

### Step 1: Validate inputs and check idempotence

Read `docs/TASKS.md` for phase `<N>`. Locate the `### Design tasks` subsection. Confirm task `<N>` exists in the subsection.
...
If either brief is missing, report `NEEDS_CONTEXT` and tell the caller to run `/arsenal-build:generate-design-briefs --phase N --task N --surface web` first.
...
If yes, spawn a fresh-context researcher subagent using the prompt template at `references/researcher-prompt.md`. Substitute the bracketed placeholders for this task's specifics. The researcher writes `.tasks/phase-{N}/task-{N}-research.md` and terminates. This skill does not read the contents.
...
- **Status:** DONE | SKIPPED (M1 idempotence) | WRONG_DOMAIN (task is `domain: feature`, route to `run-task-feature-web`) | NEEDS_CONTEXT (cite which brief is missing) | BLOCKED (cite the implementer's BLOCKED reason and your assessment, including "design brief under-specified" cases that the user may resolve via manual impeccable invocation)
- **Phase:** N
- **Task:** N
...
- **Deferred minor issues:** list (these aggregate into the phase summary at `close-design-phase-web`)
...
- **Don't paste entire docs into subagent prompts.** Briefs at `.tasks/phase-{N}/task-{N}-*.md` were generated upstream. Reference them by path.
...
| Skill | Relationship |
|---|---|
| `/arsenal-build:design-web` | Invokes this skill in a loop, one call per `- [ ]` task in the phase's `### Design tasks` subsection. Hand-off: phase number + task identifier. |
...
| `/arsenal-build:close-design-phase-web` | Runs **after** all design tasks in the phase complete. Aggregates deferred Minor issues, runs the optional `claude-in-chrome` live audit + optional impeccable audit, then hands the branch off to the feature pipeline (no push, no PR — feature close opens the single PR per phase). |
| `/arsenal-build:run-task-feature-web` | Sibling skill. Runs **after** the entire design half of the phase (including `close-design-phase-web`) completes. References the components this skill committed. |
